...
async def get_main_text(ncode: str, episode: int, async_session: AsyncSession = Depends(get_async_session)):
        """小説取得API."""
        # t-ga：小説名、全話数を出力
        payload = {
            "of": "t-ga",
        ...
        # TODO: 既読更新処理(DB連携)
        result = await async_session.execute(select(ReadHistory).where(ReadHistory.ncode == ncode))
